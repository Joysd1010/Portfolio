...
const Header = () => {
  const handleDownload = () => {
    ...
    link.href = process.env.PUBLIC_URL + downloadLink;
    link.download = 'Joy Sutradhar Resume.pdf';
    link.click();
  };
  return (
    <div className='w-full flex flex-col lg:flex-row px-4 md:px-10 gap-10 2xl:gap-20 py-20'>
      ...
      >
        ...
        <button className='bg-orange-700 text-lg text-white font-semibold rounded-md py-3 px-5' onClick={handleDownload}>
      Download CV
    </button>
          
          <a href='#contact'>
            <button className='text-lg text-white fonnt-semibold rounded-md py-3 px-5 bg-black dark:bg-[#90a7fb10]'>
               Hire Me
            </button>
          </a>
        </div>
      ...
      <div className='w-full flex-1 flex justify-center'>
        <div className='w-[300px] h-[290px] md:w-[400px] md:h-[400px] 2xl:w-[500px] 2xl:h-[500px]  border rounded-full border-gray-600 dark:border-gray-200 relative  md:mt-20 lg:mt-0 flex items-center justify-center'>
          ...
          >
            <img
              src={Reactjs}
              className='absolute top-7 md:top-20 left-3 md:-left-3 2xl:left-2 w-12 h-12 md:w-16 md:h-16' alt=""
            />
            <img
              src={Redux}
              className='absolute bottom-7 left-3 md:bottom-10 2xl:left-12 w-12 h-12 md:w-16 md:h-16' alt=""
            />
            <img
              src={Mongodb}
              className='absolute top-5 md:top-14 right-3 md:-right-3 2xl:right-2 w-12 h-12 md:w-16 md:h-16' alt=""
            />
            <img
              src={Nodejs}
              className='absolute bottom-7 right-3 md:bottom-10 2xl:right-12 w-12 h-12 md:w-16 md:h-16' alt=""
            />
          </div>
        </div>
      </div>
